...
export function App() {
  ...
  return (
    <main style={{ fontFamily: "system-ui, sans-serif", maxWidth: 1100, margin: "1.5rem auto", padding: "0 1rem" }}>
      ...
      <ConfigForm config={config} onChange={handleConfigChange} disabled={runner.status === "running" || runner.status === "paused"} />
      <Controls
        lang={config.lang}
      ...
      {runner.error && (
        <p style={{ color: "crimson" }}>
          Error: {runner.error}
          <br />
          <span style={{ color: "#888" }}>{t.unreachableHint(config.baseUrl)}</span>
        </p>
      )}
